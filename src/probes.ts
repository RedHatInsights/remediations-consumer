...
export function dispatcherRunUpdateSuccess(run_id: string, status: 'success' | 'failure' | 'running' | 'timeout' | 'canceled') {
    log.info({ run_id, status }, `dispatcher run status updated on update ${status}`);
    counters.dispatcherRun.labels('updated', status).inc();
};

export function dispatcherRunCreateSuccess(run_id: string, status: 'success' | 'failure' | 'running' | 'timeout' | 'canceled') {
    log.info({ run_id, status }, `dispatcher run status updated on create ${status}`);
    counters.dispatcherRun.labels('created', status).inc();
};
