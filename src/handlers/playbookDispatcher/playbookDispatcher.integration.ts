...
describe('playbookDispatcher handler integration tests', function () {
    test('create event with running status', async () => {
        ...
        const spy = getSandbox().spy(probes, 'dispatcherRunCreateSuccess');
    ...
    test('update to success', async () => {
        ...
        const spy = getSandbox().spy(probes, 'dispatcherRunUpdateSuccess');
    ...
    test('create event missing playbook-run label triggers error parse probe', async () => {
        await setupCreateTest();
        ...
        spy.calledOnce.should.be.true();
        spy.args[0][1].message.should.equal("Missing 'playbook-run' label in create event");
